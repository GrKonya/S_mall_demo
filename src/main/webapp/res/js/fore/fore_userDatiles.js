$(function () {
    ...
    $('#select_user_address_province').change(function () {
        $.ajax({
            type: "GET",
            url: "/schoolmall/address/" + $(this).val(),
            data: null,
            dataType: "json",
...
function uploadImage(fileDom) {
    ...
    //上传图片
    $.ajax({
        url: "/schoolmall/user/uploadUserHeadImage",
        type: "post",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (data) {
            if (data.success) {
                $(fileDom).prev("img").attr("src","/schoolmall/res/images/item/userProfilePicture/"+data.fileName);
